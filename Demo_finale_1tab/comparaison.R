          ...
          if(nrow(brut())!=0){
            ...
            plot_output_list <- lapply(1:length(variables), function(i) {
              ...
              box(title=paste("graphe de ",variables[i],sep=" "),status="info",width=6,
                  ...
                  withMathJax(radioButtons(inputId=ns(paste0("transfo2",variables[i])),label="",choiceNames=list("données brutes","log-transformation","\\(\\sqrt[4]X\\)"),choiceValues=seq(from=1, to=3)))
                  )
            })
          }
